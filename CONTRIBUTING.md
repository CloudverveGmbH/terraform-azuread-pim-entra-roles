...
- **`pim-entra-role`** — PIM for Entra directory roles (this repo).
- **`pim-azure-role`** — PIM for Azure RBAC roles.
...
equivalent change to `pim-azure-role`** so the two stay aligned. The PR template has
a checkbox for this.
...
1. Create the GitHub repo named `terraform-azuread-pim-entra-role` under `CloudverveGmbH`.
...
The Registry reference becomes `CloudverveGmbH/pim-entra-role/azuread`.
